      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.map(email => `
          <li class="participant-item">
            ...
            <button class="remove-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" type="button" aria-label="Remove ${email}">✕</button>
          </li>`).join("");
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              const response = await fetch(
                `/activities/${encodeURIComponent(activityToRemove)}/signup?email=${encodeURIComponent(emailToRemove)}`,
                { method: "DELETE" }
              );
